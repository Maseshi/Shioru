...
module.exports.command = {
    ...
    async execute(client, message, args) {
        ...
        const queueEmbed = new EmbedBuilder()
            ...
            .setURL(musicURL)
            .setDescription(queue.songs.length === 1 ? musicDisplay : musicDisplay + "\n\n" + client.translate.commands.queue.waiting_in_queue + "\n" + queueList + (queue.previousSongs.length === 1 ? "\n\n" + client.translate.commands.queue.previous_queue + "\n" + queuePreviousList : ""))
            .setColor("Blue")
            .setTimestamp(queueCreatedTimestamp)
            .setFooter({ "text": client.translate.commands.queue.owner_this_queue.replace("%s", queueAuthorUsername), "iconURL": avatarURL });
