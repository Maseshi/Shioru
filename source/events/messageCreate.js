			...
			if (argument) {
				...
				try {
					...
					const historyMessages = newLines(
						...messages.reverse().map((historyMessage) => {
							const time = new Date(
								historyMessage.createdTimestamp,
							).toLocaleString();
							const username =
								historyMessage.author.id === clientID
									? 'You'
									: (historyMessage.author.globalName ??
										historyMessage.author.username);
							const content = historyMessage.content;

							if (!historyMessage.interaction)
					...
					try {
							...
							for (let x = 0; x < promptsArray.length; x++) {
								for (let y = 0; y < promptsArray[x].length; y++) {
									if (promptsArray[x][y] === textString) {
										...
										reply =
											repliesX[Math.floor(Math.random() * repliesX.length)];
										script = scriptsX
						...
						catchError(message.client, message, 'chat', chatError, true);
					}
				}
			}
		}
