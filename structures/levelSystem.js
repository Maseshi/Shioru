                    ...
                    ref.child("config").once("value").then(function (snapshot) {
                        if (!snapshot.exists()) {
                            ...
                            if (notifyId) {
                                ...
                                levelUp.send({
                                    "embed": {
                                        "description": client.data.language.structures_levelSystem_embed_description.replace('%username', username).replace('%level', level),
                                        "color": 16312092,
                                        "thumbnail": {
                                            "url": avatar
                                        },
                                        "author": {
                                            "name": client.data.language.structures_levelSystem_embed_author_name,
